...
izpisTabele <- function(data, prvi_stolpec = "numeric"){
  library(flextable)
  library(dplyr)
  ime <- c("obdobje", "st_podatkov", "povprecje", "sd", "min", "max", "st_manj_0", "vecji", "manjsi")
  for(i in ime){
    if(i %in% (data %>% names())) {
      data[i] <- format(round(data[i]), big.mark = ".", decimal.mark = ",")  
    }
  }
  ime <- c("kolicnik", "kol_vecji", "kol_manjsi")
  for(i in ime){
    if(i %in% (data %>% names())) {
      data[i] <- format(round(data[i], 2), big.mark = ".", decimal.mark = ",")  
    }
  }
  ime <- c("lsd", "verj_izgube", "lsd_vecji", "lsd_manjsi", "psd_vecji", "psd_manjsi", "psd")
  for(i in ime){
    if(i %in% (data %>% names())) {
...
##########################
# manjsi in vecji trendi #
##########################

# povprečje in povprečna stopnja donosa pri manjših in večjih trendih
manj_vec <- function(l1, l2, l3, l4, enota){
  manjsi <- c(mean(l1[500:1500]), mean(l2[500:1500]), mean(l3[500:1000]), mean(c(l4[1:300], l4[400:length(l4)])))
  vecji <- c(mean(c(l1[1:499], l1[1501:length(l1)])), mean(c(l2[1:499], l2[1501:length(l2)])), 
             mean(c(l3[1:499], l3[1001:length(l3)])), mean(l4[301:399]))
  cagr_manjsi <- c(cagr(manjsi[1], obdobje = 360), cagr(manjsi[2], obdobje = 500), 
                      cagr(manjsi[3], obdobje = 1000), cagr(manjsi[4]))
  cagr_vecji <- c(cagr(vecji[1], obdobje = 360), cagr(vecji[2], obdobje = 500), 
                    cagr(vecji[3], obdobje = 1000), cagr(vecji[4]))
  if(enota == "d"){
    data.frame("obdobje" = c(360, 500, 1000, 1800), 
             "vecji" = vecji, "manjsi" = manjsi, 
             "lsd_vecji" = cagr_vecji, 
             "lsd_manjsi" = cagr_manjsi)
  }
  else{
    data.frame("obdobje" = c(360, 500, 1000, 1800), 
               "vecji" = vecji, "manjsi" = manjsi, 
               "psd_vecji" = cagr_vecji, 
               "psd_manjsi" = cagr_manjsi)
  }
}

# 1day
S1_manj_vec <- manj_vec(btc_360_S1, btc_500_S1, btc_1000_S1, btc_1800_S1, "d")
...
# Standardna deviacija pri manjših in večjih trendih
sd_manj_vec <- function(l1, l2, l3, l4){
  manjsi <- c(mean(l1[500:1500]), mean(l2[500:1500]), mean(l3[500:1000]), mean(c(l4[1:300], l4[400:length(l4)])))
  vecji <- c(mean(c(l1[1:499], l1[1501:length(l1)])), mean(c(l2[1:499], l2[1501:length(l2)])), 
             mean(c(l3[1:499], l3[1001:length(l3)])), mean(l4[301:399]))
  sd_manjsi <- c(sd(l1[500:1500]), sd(l2[500:1500]), sd(l3[500:1000]), sd(c(l4[1:300], l4[400:length(l4)])))
  sd_vecji <- c(sd(c(l1[1:499], l1[1501:length(l1)])), sd(c(l2[1:499], l2[1501:length(l2)])), 
             sd(c(l3[1:499], l3[1001:length(l3)])), sd(l4[301:399]))
  kolicnik_manjsi <- manjsi/sd_manjsi
  kolicnik_vecji <- vecji/sd_vecji
  data.frame("obdobje" = c(360, 500, 1000, 1800), 
             "vecji" = sd_vecji, "manjsi" = sd_manjsi, 
             "kol_vecji" = kolicnik_vecji, "kol_manjsi" = kolicnik_manjsi)
}

S1sd_manj_vec <- sd_manj_vec(btc_360_S1, btc_500_S1, btc_1000_S1, btc_1800_S1)
#1h
S1sd_1h_manj_vec <- sd_manj_vec(btc_1h_360_S1, btc_1h_500_S1, btc_1h_1000_S1, 
                                        btc_1h_1800_S1, 17530)
#5min
S1sd_5min_manj_vec <- sd_manj_vec(btc_5min_360_S1, btc_5min_500_S1, btc_5min_1000_S1, 
                                          btc_5min_1800_S1, 210710)
...
izpisMatrike <- function(data, kaj = "povprecje"){
  ...
  if("kolicnik" %in% (data %>% rownames())) {
   for(j in 2:c){
     ...
     for(i in 1:(n-1)){
       data[i,j] <- format(as.numeric(data[i,j]), big.mark = ".", decimal.mark = ",")
     }
   }
  }
  else{
...
indikatorji %>% izpisTabele(prvi_stolpec = "character")
